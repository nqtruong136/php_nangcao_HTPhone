<?php
// echo $TongQuanChiTiet['getMainProductInfo'][0]['TenSanPham'];
$product = $TongQuanChiTiet['getMainProductInfo'][0];
$variants = $TongQuanChiTiet['getVariants'];
$galleryImages = $TongQuanChiTiet['getGalleryImages'];

$uniqueColors = array_unique(array_column($variants, 'MauSac'));
$uniqueSizes = array_unique(array_column($variants, 'DungLuong'));
  ...
  <section class="section-box shop-template">
    <div class="container">
      <div class="row">
        ...
        <div class="col-lg-7">
          <h3 class="color-brand-3 mb-25"><?php echo htmlspecialchars($product['TenSanPham']);
                                          echo " ", htmlspecialchars($product['TieuDePhu']); ?></h3>
          <div class="row align-items-center">

            <div class="col-lg-4 col-md-4 col-sm-3 mb-mobile"><span class="bytext color-gray-500 font-xs font-medium">by</span><a class="byAUthor color-gray-900 font-xs font-medium" href="shop-vendor-single.html"><?php echo htmlspecialchars($product['TenNhaCungCap']); ?></a>
          ...
          <div class="row">
            <div class="col-lg-7">
              <div class="box-product-price">
                <?php if (!empty($defaultVariant['GiaKhuyenMai'])): ?>
                  <h3 class="color-brand-3 price-main d-inline-block mr-10"><?php echo number_format($defaultVariant['GiaKhuyenMai']); ?>đ</h3>
                  <span class="color-gray-500 price-line font-xl line-througt"><?php echo number_format($defaultVariant['GiaGoc']); ?>đ</span>
                <?php else: ?>
                  <h3 class="color-brand-3 price-main d-inline-block mr-10"><?php echo number_format($defaultVariant['GiaGoc']); ?>đ</h3>
                <?php endif; ?>
              </div>
              <div class="box-progress-product mt-15 mb-20">
                <div class="progress mb-5">
                  <div class="progress-bar" role="progressbar" style="width: 50%" aria-valuenow="<?php echo number_format((int)$defaultVariant['SoLuotBan'] / (int)$defaultVariant['SoLuongTonKho'] * 100); ?>" aria-valuemin="0" aria-valuemax="100"></div>
                </div><span class="font-xs color-gray-500">Sold: <?php echo number_format($defaultVariant['SoLuotBan']); ?>/<?php echo number_format($defaultVariant['SoLuongTonKho']); ?></span>
              </div>
              <div class="product-description color-gray-900">
                <?php
                // Dữ liệu đã là HTML nên ta echo trực tiếp
                // Lưu ý: Trong dự án thực tế, cần lọc để chống lỗi bảo mật XSS
                echo $product['MoTaNgan_HTML'];
                ?>
              </div>
              <div class="border-bottom mt-20 mb-20"></div>
              <div class="info-product">
                <div class="row align-items-end">
                  <div class="col-lg-6 col-md-6 mb-20"><span class="font-sm font-medium color-gray-900">SKU:<span class="color-gray-500">iphone12pro128</span><br>Category:<span class="color-gray-500">Smartphones</span><br>Tags:<span class="color-gray-500">Blue, Smartphone</span></span>
                  </div>
            ...
            <div class="col-lg-5">
              <div class="box-border-product">
                <div class="box-product-color">
                  <p class="font-sm color-gray-900">Color:<span class="color-brand-2 nameColor"><?php echo htmlspecialchars($defaultVariant['MauSac']); ?></span></p>
                  <ul class="list-colors">
                    <?php foreach ($uniqueColors as $color): ?>
                      <li class="<?php echo ($color === $defaultVariant['MauSac']) ? 'active' : ''; ?>"
                        onclick="selectVariantOption('color', '<?php echo $color; ?>')">
                ...
                <div class="box-product-style-size mt-20">
                  <div class="row">
                    <?php if (!empty($uniqueStyles)): ?>
                      <div class="col-lg-12 mb-20">
                        <p class="font-sm color-gray-900">Style:
                          <span class="color-brand-2 nameStyle"><?php echo htmlspecialchars($defaultVariant['Style']); ?></span>
                        </p>
                        <ul class="list-styles">
                          <?php foreach ($uniqueStyles as $style): ?>
                            <li class="<?php echo ($style === $defaultVariant['Style']) ? 'active' : ''; ?>"
                              onclick="selectVariantOption('style', '<?php echo $style; ?>')">
                    ...
                    <div class="col-lg-12 mb-10">
                      <p class="font-sm color-gray-900">Size:<span class="color-brand-2 nameSize"><?php echo htmlspecialchars($defaultVariant['DungLuong']); ?></span></p>
                      <ul class="list-sizes">
                        <?php foreach ($uniqueSizes as $size): ?>
                          <li class="<?php echo ($size === $defaultVariant['DungLuong']) ? 'active' : ''; ?>"
                            onclick="selectVariantOption('size', '<?php echo $size; ?>')">
        ...
        <div class="tab-content">
          <div class="tab-pane fade active show" id="tab-description" role="tabpanel" aria-labelledby="tab-description">
            <div class="display-text-short">
              <p>
                <?php
                // Lấy mô tả chi tiết từ mảng dữ liệu
                $moTa = $tab4['TabFull'][0]['MoTaChiTiet'];
                echo $moTa; // In ra nội dung HTML
                ?>
              </p>
          ...
          <div class="tab-pane fade" id="tab-specification" role="tabpanel" aria-labelledby="tab-specification">
            ...
            <?php $product_specs = $tab4['TabFull'][0]; ?>
            <div class="table-responsive">
              <table class="table table-striped">
                <?php if (!empty($product_specs['MaSanPham'])): ?>
                  <tr>
                    <td>Mã Sản Phẩm</td>
                    <td><?php echo htmlspecialchars($product_specs['MaSanPham']); ?></td>
                  </tr>
                <?php endif; ?>
                <?php if (!empty($product_specs['TenNhaCungCap'])): ?>
                  <tr>
                    <td>Tên Nhà Cung Cấp</td>
                    <td><?php echo htmlspecialchars($product_specs['TenNhaCungCap']); ?></td>
                  </tr>
                <?php endif; ?>
                <?php if (!empty($product_specs['ManHinhRong'])): ?>
                  <tr>
                    <td>Màn Hình Rộng</td>
                    <td><?php echo htmlspecialchars($product_specs['ManHinhRong']); ?></td>
                  </tr>
                <?php endif; ?>
                <?php if (!empty($product_specs['CongNgheManHinh'])): ?>
                  <tr>
                    <td>Công Nghệ Màn Hình</td>
                    <td><?php echo htmlspecialchars($product_specs['CongNgheManHinh']); ?></td>
                  </tr>
                <?php endif; ?>

                <?php if (!empty($product_specs['DoPhanGiai'])): ?>
                  <tr>
                    <td>Độ Phân Giải</td>
                    <td><?php echo htmlspecialchars($product_specs['DoPhanGiai']); ?></td>
                  </tr>
                <?php endif; ?>

                <?php if (!empty($product_specs['ChipXuLy'])): ?>
                  <tr>
                    <td>Chip Xử Lý</td>
                    <td><?php echo htmlspecialchars($product_specs['ChipXuLy']); ?></td>
                  </tr>
                <?php endif; ?>

                <?php if (!empty($product_specs['Ram'])): ?>
                  <tr>
                    <td>RAM</td>
                    <td><?php echo htmlspecialchars($product_specs['Ram']); ?></td>
                  </tr>
                <?php endif; ?>

                <?php if (!empty($product_specs['DungLuongPin'])): ?>
                  <tr>
                    <td>Dung Lượng Pin</td>
                    <td><?php echo htmlspecialchars($product_specs['DungLuongPin']); ?></td>
                  </tr>
                <?php endif; ?>
          ...
          <div class="tab-pane fade" id="tab-additional" role="tabpanel" aria-labelledby="tab-additional">
            ...
            <div class="table-responsive">
              <table class="table table-striped">
                <tbody>
                  <?php if (!empty($product_specs['TrongLuong'])): ?>
                    <tr>
                      <td>Trọng Lượng</td>
                      <td>
                        <p><?php echo htmlspecialchars($product_specs['TrongLuong']); ?></p>
                      </td>
                    </tr>
                  <?php endif; ?>
                  <tr>
                  ...
                  <?php if (!empty($product_specs['KichThuoc'])): ?>
                    <tr>
                      <td>Kích Thước</td>
                      <td>
                        <p><?php echo htmlspecialchars($product_specs['KichThuoc']); ?></p>
                      </td>
                    </tr>
                  <?php endif; ?>
                  <!-- <tr>
          ...
          <div class="tab-pane fade" id="tab-reviews" role="tabpanel" aria-labelledby="tab-reviews">
            <?php
            $reviews = $tab4['ReviewTab'];
            $stats = $tab4['StarCommentTab'][0];
            ?>

            <div class="comments-area">
              <div class="row">
                <div class="col-lg-8">
                  <h4 class="mb-30 title-question">Customer questions &amp; answers (<?php echo count($reviews); ?>)</h4>
                        ...
                        <div class="single-comment justify-content-between d-flex mb-30 hover-up">
                          <div class="user justify-content-between d-flex">
                            <div class="thumb text-center">
                              <!-- Ảnh đại diện của người dùng, em có thể thêm cột này vào bảng Users sau -->
                              <img src="assets/imgs/page/product/<?php echo htmlspecialchars($review['AnhDaiDien']); ?>" alt="Ecom">
                              <a class="font-heading text-brand" href="#"><?php echo htmlspecialchars($review['TenUser']); ?></a>
                            </div>
                            <div class="desc">
                              <div class="d-flex justify-content-between mb-10">
                                <div class="d-flex align-items-center">
                                  <!-- CSDL của mình chưa có ngày đánh giá, em có thể thêm cột ThoiGian vào bảng Reviews sau -->
                                  <span class="font-xs color-gray-700">
                                    <?php
                                    $dt = new DateTime($review['ThoiGian']);
                                    echo $dt->format('F j, Y \a\t g:i a');
                                    ?>
                                  </span>
                                </div>
                                <div class="product-rate d-inline-block">
                                  <!-- Tính toán độ rộng của thanh sao -->
                                  <div class="product-rating" style="width: <?php echo ($review['SoSao'] / 5) * 100; ?>%"></div>
                                </div>
                              </div>
                ...
                <div class="col-lg-4">
                  ...
                  <div class="d-flex mb-30">
                    <div class="product-rate d-inline-block mr-15">
                      <div class="product-rating" style="width: <?php echo ($stats['DiemTrungBinh'] / 5) * 100; ?>%"></div>
                    </div>
                    <h6><?php echo $stats['DiemTrungBinh']; ?> out of 5</h6>
                  </div>
                  <?php
                  // Hàm trợ giúp để tính phần trăm, tránh lỗi chia cho 0
                  ...
                  $totalReviews = $stats['TongSoDanhGia'];
                  $fiveStarPercent = calculate_percentage($stats['SoLuong5Sao'], $totalReviews);
                  $fourStarPercent = calculate_percentage($stats['SoLuong4Sao'], $totalReviews);
                  $threeStarPercent = calculate_percentage($stats['SoLuong3Sao'], $totalReviews);
                  $twoStarPercent = calculate_percentage($stats['SoLuong2Sao'], $totalReviews);
                  $oneStarPercent = calculate_percentage($stats['SoLuong1Sao'], $totalReviews);
                  ?>
                  <div class="progress"><span>5 star</span>
                    <div class="progress-bar" role="progressbar" style="width: <?php echo $fiveStarPercent; ?>%" aria-valuenow="<?php echo $fiveStarPercent; ?>" aria-valuemin="0" aria-valuemax="100"><?php echo $fiveStarPercent; ?>%</div>
                  </div>
                  <div class="progress"><span>4 star</span>
                    <div class="progress-bar" role="progressbar" style="width: <?php echo $fourStarPercent; ?>%" aria-valuenow="<?php echo $fourStarPercent; ?>" aria-valuemin="0" aria-valuemax="100"><?php echo $fourStarPercent; ?>%</div>
                  </div>
                  <div class="progress"><span>3 star</span>
                    <div class="progress-bar" role="progressbar" style="width: <?php echo $threeStarPercent; ?>%" aria-valuenow="<?php echo $threeStarPercent; ?>" aria-valuemin="0" aria-valuemax="100"><?php echo $threeStarPercent; ?>%</div>
                  </div>
                  <div class="progress"><span>2 star</span>
                    <div class="progress-bar" role="progressbar" style="width: <?php echo $twoStarPercent; ?>%" aria-valuenow="<?php echo $twoStarPercent; ?>" aria-valuemin="0" aria-valuemax="100"><?php echo $twoStarPercent; ?>%</div>
                  </div>
                  <div class="progress mb-30"><span>1 star</span>
                    <div class="progress-bar" role="progressbar" style="width: <?php echo $oneStarPercent; ?>%" aria-valuenow="<?php echo $oneStarPercent; ?>" aria-valuemin="0" aria-valuemax="100"><?php echo $oneStarPercent; ?>%</div>
          ...
          <div class="tab-pane fade" id="tab-vendor" role="tabpanel" aria-labelledby="tab-vendor">
            <?php
            // Dữ liệu vendor đã có sẵn trong mảng thông tin chính của sản phẩm
            $vendor = $tab4['TabFull'][0];
            ?>
            <div class="vendor-logo d-flex mb-30"><img src="assets/imgs/page/product/<?php echo htmlspecialchars($vendor['LogoURL']); ?>" alt="">
              <div class="vendor-name ml-15">
                <h6><a href="shop-vendor-single.html"><?php echo htmlspecialchars($vendor['TenNhaCungCap']); ?></a></h6>
                <div class="product-rate-cover text-end">
                  <div class="product-rate d-inline-block">
                    <div class="product-rating" style="width: <?php echo $vendor['RatingPhanTram']; ?>%"></div>
                  </div><span class="font-small ml-5 text-muted"> (<?php echo $vendor['TongLuotDanhGia']; ?> reviews)</span>
                </div>
              </div>
            </div>
            <ul class="contact-infor mb-50">
              <li><img src="assets/imgs/page/product/icon-location.svg" alt=""><strong>Address:</strong><span> <?php echo htmlspecialchars($vendor['DiaChi']); ?></span></li>
              <li><img src="assets/imgs/page/product/icon-contact.svg" alt=""><strong>Contact Seller:</strong><span> <?php echo htmlspecialchars($vendor['SoDienThoai']); ?></span></li>
            </ul>
            <div class="d-flex mb-25">
              <div class="mr-30">
                <p class="color-brand-1 font-xs">Rating</p>
                <h4 class="mb-0"><?php echo $vendor['RatingPhanTram']; ?>%</h4>
              </div>
              <div class="mr-30">
                <p class="color-brand-1 font-xs">Ship on time</p>
                <h4 class="mb-0"><?php echo $vendor['ShipOnTimePhanTram']; ?>%</h4>
              </div>
              <div>
                <p class="color-brand-1 font-xs">Chat response</p>
                <h4 class="mb-0"><?php echo $vendor['ChatResponsePhanTram']; ?>%</h4>
              </div>
            </div>
...
<script>
  ...
  const currentProductId = <?php echo json_encode($_GET['id']); ?>;

  // Gọi hàm để lưu ID này vào localStorage
...
<script>
  ...
  let currentSelection = {
    color: '<?php echo $defaultVariant['MauSac']; ?>',
    size: '<?php echo $defaultVariant['DungLuong']; ?>'
  };

  // Hàm này được gọi mỗi khi người dùng click vào một tùy chọn
  function selectVariantOption(optionType, value) {
    if (optionType === 'color') {
      currentSelection.color = value;
    } else if (optionType === 'size') {
      currentSelection.size = value;
    }

    // Tìm phiên bản phù hợp với lựa chọn hiện tại
    let selectedVariant = findSelectedVariant();

    // Cập nhật giao diện với thông tin của phiên bản đã chọn
    updateProductView(selectedVariant);

    // Cập nhật lại trạng thái active cho các nút
    updateActiveStates();
  }

  // Hàm tìm kiếm trong mảng allVariants
  function findSelectedVariant() {
    return allVariants.find(variant => {
      return variant.MauSac === currentSelection.color && variant.DungLuong === currentSelection.size;
    });
  }

  // Hàm cập nhật các thành phần trên trang
  function updateProductView(variant) {
    const priceMainEl = document.querySelector('.price-main');
    const priceLineEl = document.querySelector('.price-line');
    const nameColorEl = document.querySelector('.nameColor');
    const nameSizeEl = document.querySelector('.nameSize');
    // ... thêm các element khác em muốn cập nhật (SKU, progress bar,...)

    if (variant) {
      // Nếu tìm thấy phiên bản hợp lệ
      let finalPrice = variant.GiaKhuyenMai ? variant.GiaKhuyenMai : variant.GiaGoc;
      priceMainEl.innerText = new Intl.NumberFormat('vi-VN').format(finalPrice) + 'đ';

      if (variant.GiaKhuyenMai) {
        priceLineEl.innerText = new Intl.NumberFormat('vi-VN').format(variant.GiaGoc) + 'đ';
        priceLineEl.style.display = 'inline';
      } else {
        priceLineEl.style.display = 'none';
      }

      nameColorEl.innerText = variant.MauSac;
      nameSizeEl.innerText = variant.DungLuong;

      // Cập nhật cả nút "Add to Cart" để nó biết cần thêm phiên bản nào
      // Ví dụ: document.getElementById('add-to-cart-btn').dataset.variantId = variant.MaBienThe;
    } else {
      // Nếu không có phiên bản này (ví dụ: không có màu Xanh size 1TB)
      priceMainEl.innerText = "Không có sẵn";
      priceLineEl.style.display = 'none';
      // Vô hiệu hóa nút "Add to cart"
    }
  }

  // Hàm cập nhật giao diện nút nào đang được chọn
  function updateActiveStates() {
    document.querySelectorAll('.list-colors li').forEach(li => {
      if (li.innerText === currentSelection.color) {
    ...
    document.querySelectorAll('.list-sizes li').forEach(li => {
      if (li.innerText === currentSelection.size) {
        li.classList.add('active');
      } else {
        li.classList.remove('active');
      }
